...
if (isset($_POST["updateTodo"])) {
    ...
    // Updating todo
    $sql = "UPDATE tasks SET title='{$title}', date='{$date}', time='{$time}', comment='{$comment}' WHERE id='{$todoId}'";
...
$sql = "SELECT * FROM tasks WHERE id='{$todoId}' AND user_id='{$user_id}'";
$res = mysqli_query($conn, $sql);
                    ...
                    <div class="card-body p-4">
                        ...
                        <form action="" method="POST">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="date" class="form-control" id="date" name="date" placeholder="e.g. Create a PHP program" value="<?php echo $todoData['date']; ?>" required>
                            </div>
                            <div class="mb-3">
                                <label for="title" class="form-label">Time(seconds)</label>
                                <input type="text" class="form-control" id="time" name="time" placeholder="e.g. Create a PHP program" value="<?php echo $todoData['time']; ?>" required>
                            </div>
                            <div class="mb-3">
                                <label for="desc" class="form-label">Comment</label>
                                <textarea class="form-control" id="comment" name="comment" rows="3" required><?php echo $todoData['comment']; ?></textarea>
                            </div>
                            <div>
